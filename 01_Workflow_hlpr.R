...
get.wind.speed = function(canopy.height, wrf, height.dir, file.base, dim1_index, dim2_index){
  ...
  if (canopy.height > 10){
    ...
    if (canopy.height <= heights[1]){
      height = heights[1]
      U = ncvar_get(wrf, "U", start = c(dim1_index, dim2_index, 1, 1), count = c(1,1,1,24)) # (dim1, dim2, layer, time)
      V = ncvar_get(wrf, "V", start = c(dim1_index, dim2_index, 1, 1), count = c(1,1,1,24))
      
      height.wind.speed = sqrt(U^2 + V^2)
      
    }else{
      height = heights[2]
      U = ncvar_get(wrf, "U", start = c(dim1_index, dim2_index, 2, 1), count = c(1,1,1,24)) # (dim1, dim2, layer, time)
      V = ncvar_get(wrf, "V", start = c(dim1_index, dim2_index, 2, 1), count = c(1,1,1,24))
